...
Withdraw Transaction Create Template
    ...
    @{args}    Create List    ${l_initial_with_amt+${l_data['withAmount']['input']}}    ${l_post_with_amt}    
    ...
    @{args}    Create List    ${initial_balance-${l_data['withAmount']['input']}}    ${post_balance}
    VERIFY    Should Be Equal    ${args}
    ...    i_pass_message=Client balance updated correctly.
    ...    i_fail_message=Client balance not updated correctly.       
    
Withdraw Transaction View Template
    [Arguments]    ${i_data}
    ${l_data}    Load Json Data Into Dictionary    ${i_data}
    ...
    Form Withdraw Transaction Verify Fields    ${l_data['client']['verify']}    ${l_data['withAmount']['verify']}
